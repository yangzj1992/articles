...
建议使用requestAnimationFrame
定义绘制每一帧前的工作。 requestAnimationFrame(callback)
方法可以自动调节频率。callback工作太多时无法在一帧内完成，会自动降低为30FPS, 虽然频率会降低但比丢帧好。
渲染一帧目标（1/60FPS = 16ms-)
...
### 改良动画队列
添加了animator 是否为 Animator 的实例判断
```
function AnimationQueue(animators){
  this.animators = animators || [];
}
